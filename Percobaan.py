...
# --- Fungsi untuk Visualisasi BST di Terminal (Teks) ---
def print_tree_terminal(node, prefix="", is_left=True, level_char="    "):
    """Mencetak struktur pohon ke terminal."""
    if node is not None:
        display_nama_obat = node.nama_obat.title()
        print(prefix + ("|-- " if not is_left else "L-- ") + f"{display_nama_obat} (Data: {len(node.data)})")
        # print(prefix + ("|-- " if not is_left else "L-- ") + node.nama_obat + f" (Data: {len(node.data)})")
        new_prefix = prefix + (level_char if not is_left else level_char)
        if node.left is not None or node.right is not None:
            # Beri spasi ekstra jika ada anak, untuk garis vertikal
            new_prefix_for_children = prefix + ("|   " if is_left else "    ")
            if node.right: # Cetak anak kanan dulu agar struktur pohon lebih intuitif di terminal
                 print_tree_terminal(node.right, new_prefix_for_children, False, level_char)
            if node.left:
                 print_tree_terminal(node.left, new_prefix_for_children, True, level_char)
...
def _assign_positions_recursive(node, x=0, y=0, x_dist=1.0, y_dist=1.0, positions=None, depth=0, x_coords_at_depth=None):
    ...
    # Tentukan x untuk anak kiri
    left_x_offset = x
    if node.left:
        left_x_offset = _assign_positions_recursive(node.left, x, y - y_dist, x_dist / 2, y_dist, positions, depth + 1, x_coords_at_depth)
    
    # Tentukan x untuk node saat ini
    # Coba tempatkan node di tengah antara anak kiri dan awal dari anak kanan
    current_x = left_x_offset
    if node.left: # Jika ada anak kiri, geser node saat ini ke kanan sedikit
        current_x += x_dist / (2**(depth+1)) # Penyesuaian agar tidak terlalu rapat

    # Hindari tumpang tindih pada kedalaman yang sama
    while x_coords_at_depth.get(depth, -float('inf')) >= current_x :
        current_x += 0.2 # Sedikit penyesuaian jika ada tumpang tindih

    node.x = current_x
    node.y = y
    positions[node.nama_obat] = (node.x, node.y)
    x_coords_at_depth[depth] = current_x # Catat x terakhir pada kedalaman ini

    # Tentukan x untuk anak kanan
    right_x_start = current_x
    if node.right:
        if node.left: # Jika ada anak kiri, beri jarak lebih untuk anak kanan
            right_x_start += x_dist / (2**(depth+1))
        else: # Jika tidak ada anak kiri, anak kanan mulai dari posisi node saat ini + sedikit jarak
            right_x_start += x_dist / (2**(depth+1)) 
...
def visualize_bst_matplotlib(tree_root, filename="bst_matplotlib.png", title="Visualisasi BST"):
    ...
    if tree_root is None:
        ...
        # Membuat gambar kosong dengan teks
    ...
    fig, ax = plt.subplots(figsize=(15, 10)) # Ukuran bisa disesuaikan
    
    # Mengassign posisi x dan y ke setiap node
    # Pendekatan ini sederhana dan mungkin memerlukan penyesuaian untuk pohon besar/tidak seimbang
    # Untuk layout yang lebih canggih, pustaka seperti networkx sering digunakan
    node_positions = {}
    x_coords_by_depth = {} # Untuk membantu mencegah tumpang tindih horizontal
    
    # Fungsi untuk menghitung lebar subtree (jumlah node dalam inorder traversal)
    def get_subtree_width(node):
        if not node: return 0
        return get_subtree_width(node.left) + 1 + get_subtree_width(node.right)

    # Fungsi untuk assign posisi, mencoba menyebarkan node
    # Ini adalah heuristik sederhana, mungkin tidak sempurna untuk semua kasus
    max_depth = [0]
    ...
    # Posisi x akan didasarkan pada inorder traversal untuk urutan, dengan penyesuaian
    # Posisi y akan didasarkan pada kedalaman
    current_x = [0] # Menggunakan list agar bisa diubah dalam rekursi (pass-by-reference-like)
    def assign_xy(node, depth=0, y_spacing=1):
        ...
        current_x[0] += 1 # Increment x untuk node berikutnya dalam inorder
        
        assign_xy(node.right, depth + 1, y_spacing)

    assign_xy(tree_root) # Panggil fungsi untuk mengassign x dan y
...
if __name__ == "__main__":
    print("\n--- Visualisasi BST Keseluruhan (Terminal) ---")
    if root:
        print_tree_terminal(root)
    else:
        print("BST kosong.")
    ...
    print(f"\n--- Mencari '{nama_obat_dicari}' dan Visualisasi Subtree-nya ---")
    node_hasil_pencarian = search(root, nama_obat_dicari)

    if node_hasil_pencarian:
        print(f"\nSubtree untuk '{nama_obat_dicari.title()}' (Terminal):")
        print_tree_terminal(node_hasil_pencarian)
    ...
    print("Visualisasi pohon telah dicetak di terminal dan/atau disimpan sebagai file gambar.")
    demo.launch()
